...
const ArticleGeneratePage: React.FC<ArticleGeneratePageProps> = ({
  ...
  const [scrapModalTop, setScrapModalTop] = useState<number>(
    DEFAULT_MODAL_TOP_OFFSET
  );
  const SIDE_RAIL_WIDTH = 60; // Header에 추가된 사이드바 최소 폭과 동일하게 유지

  const [isStyleDropdownOpen, setIsStyleDropdownOpen] =
  ...
  // 참고 자료 모달 탭 상태
  const [referenceModalTab, setReferenceModalTab] = useState<'SCRAP' | 'PDF'>(
    'SCRAP'
  );
  // 모달 취소 복구를 위한 백업 상태
  const backupSelectedScrapsRef = useRef<ScrapResponse[]>([]);
  const backupSelectedUploadsRef = useRef<
    Array<{ uploadedFileId: number; title: string; usagePrompt: string }>
  >([]);
  // 탭별 태그 필터(탭 이동 시 초기화)
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (isScrapModalOpen) {
        const modalOverlay = document.querySelector(
          `.${articleStyles.modalOverlay}`
        );
        const scrapModal = document.querySelector(
          `.${articleStyles.scrapModal}`
        );

        if (
          modalOverlay &&
          target === modalOverlay &&
          !scrapModal?.contains(target)
        ) {
          toggleScrapModal();
        }
      }
      ...
      // 태그 목록 툴팁 처리
      if (showAllTags !== null) {
        const tagListTooltip = document.querySelector(
          `[data-taglist-id="${showAllTags}"]`
        );
        if (
          tagListTooltip &&
          !tagListTooltip.contains(target) &&
          !(
            target instanceof HTMLButtonElement &&
            target.classList.contains(styles.moreTag)
          )
        ) {
          setShowAllTags(null);
        }
    ...
    if (isGenerating) {
      ...
      generationTimerRef.current = setInterval(() => {
        const secs = Math.floor(
          (Date.now() - generationStartTimeRef.current) / 1000
        );
  ...
  const handleGenerateTemplateFromPage = async () => {
    ...
    try {
      ...
      showInfo(
        t('articleGenerate_pageAnalysis'),
        t('articleGenerate_analysisDescription')
      );
      ...
      if (!response.success) {
        throw new Error(
          response.error || t('articleGenerate_pageContentFetchFailed')
        );
      }
      ...
      showSuccess(
        t('articleGenerate_aiAnalysisComplete'),
        t('articleGenerate_sectionsCreated').replace(
          '{count}',
      ...
      showError(
        t('articleGenerate_sectionGenerationFailed'),
        error.message || t('articleGenerate_sectionGenerationError')
      );
    } finally {
      setAnalyzing(false);
    }
  ...
  const addSection = async (parentId?: string) => {
    const newTitle = parentId
      ? t('articleGenerate_newSubsection')
      : t('articleGenerate_newSection');
  ...
  const handleGenerateArticle = async () => {
    ...
    try {
      await trackArticleGenerationStartedBridge({
        ...
        template_sections_count: templateStructure
    ...
    const structuredIdeas = templateStructure
      ? collectIdeas(templateStructure)
      : {};
    const isTemplateMode =
      templateStructure &&
      Object.values(structuredIdeas).some(
        (idea: any) => typeof idea === 'string' && idea.trim() !== ''
      );
    ...
    if (isTemplateMode) {
      if (
        ...
        showError(
          t('articleGenerate_inputError'),
          t('articleGenerate_enterSectionIdeas')
        );
        return;
      }
    } else {
      if (!topic || !keyInsight) {
        ...
        showError(
          t('articleGenerate_inputError'),
          t('articleGenerate_enterTopicAndKeyMessage')
        );
        return;
      }
    }
    ...
    try {
      ...
      const templateWithoutIds = templateStructure
        ? removeIdsFromTemplate(templateStructure)
        : [];

      // V3 API를 사용한 비동기 생성 (PDF 지원)
      const generateData: GenerateArticleV3Dto = {
        topic: isTemplateMode
          ? templateStructure?.[0]?.title ||
            t('articleGenerate_sectionBasedArticle')
          : topic,
        keyInsight: isTemplateMode
          ? JSON.stringify(structuredIdeas)
          : keyInsight,
      ...
      articleService
        .generateArticleV3(generateData)
        .then(async (response) => {
          // 즉시 요청 성공 메시지 표시
          showInfo(
            t('articleGenerate_draftGenerationStarted'),
            `${t('articleGenerate_articleGenerationStarted')}${selectedUploads.length > 0 ? t('articleGenerate_pdfAnalysisIncluded').replace('{count}', selectedUploads.length.toString()) : ''}`
          );

          try {
            // 백그라운드에서 완성 대기 (최대 50회, 5초 간격 = 2.5분)
            const completedArticle =
              await articleService.waitForArticleCompletionV3(
                response.articleId,
                50,
                5000
              );
            ...
            if (completedArticle.status === 'completed') {
              ...
              setGenerating(false);
              const serverMsg = (completedArticle as any)?.errorMessage as
                | string
                | undefined;
              const hint = t('articleGenerate_generationHint');
              setGenerationError(serverMsg || hint);
              showError(
                t('articleGenerate_draftGenerationFailed'),
                serverMsg || hint
              );
            }
          } catch (pollingError) {
            ...
            setGenerating(false);
            showError(
              t('articleGenerate_statusCheckFailed'),
              t('articleGenerate_checkArchiveManually')
            );
          }
        })
        .catch((error) => {
          ...
          showError(
            t('articleGenerate_draftGenerationFailed'),
            error.message || t('articleGenerate_requestFailed')
          );
        });
    } catch (error: any) {
      setGenerationError(
        error.message || t('articleGenerate_draftGenerationFailedGeneral')
      );
      setGenerationStatus('failed');
      setGenerating(false);
      showError(
        t('articleGenerate_requestSendFailed'),
        error.message || t('articleGenerate_requestError')
      );
    }
  };
  ...
  const toggleUploadSelection = async (upload: LibraryItemDto) => {
    setSelectedUploads((prev) => {
      ...
      return [
        ...prev,
        { uploadedFileId: upload.id, title: upload.title, usagePrompt: '' },
      ];
    });

    const isSelected = selectedUploads.find(
      (u) => u.uploadedFileId === upload.id
    );
    try {
      if (isSelected) {
        await trackArticleReferenceRemovedBridge({
  ...
  const setUploadUsagePrompt = (uploadedFileId: number, value: string) => {
    setSelectedUploads((prev) =>
      prev.map((u) =>
        u.uploadedFileId === uploadedFileId ? { ...u, usagePrompt: value } : u
      )
    );
  };
  ...
  useEffect(() => {
    const fetchScraps = async () => {
      ...
      setAllTags(
        Array.from(
          new Set(
            scraps.flatMap((scrap) => scrap.tags?.map((tag) => tag.name) || [])
          )
        ).sort()
      );
    };
  ...
  useEffect(() => {
    const fetchUploads = async () => {
      try {
        ...
        const pdfs = uploads.filter(
          (u: LibraryItemDto) =>
            u.mimeType?.includes('pdf') || u.url?.toLowerCase().endsWith('.pdf')
        );
        setAllUploads(pdfs);
        setAllUploadTags(
          Array.from(
            new Set(pdfs.flatMap((u: LibraryItemDto) => u.tags || []))
          ).sort()
        );
      } catch (err) {
  ...
  const filteredUploads = useMemo(() => {
    ...
    return allUploads.filter((u: LibraryItemDto) =>
      (u.tags || []).some((t: string) => uploadTagFilters.includes(t))
    );
        ...
        <div className={articleStyles.scrollableContent}>
          ...
          <div className={styles.draftForm}>
            ...
            <div className={styles.referenceSection}>
              <div className={articleStyles.sectionHeader}>
                ...
                <div className={articleStyles.sectionActions}>
                  ...
                  >
                    ...
                    {isAnalyzing
                      ? t('articleGenerate_analyzing')
                      : t('articleGenerate_currentPageSectionAnalysis')}
                  </button>
                </div>
              </div>
              ...
              {!templateStructure && (
                <div className={articleStyles.emptyState}>
                  <RiAiGenerate
                    size={24}
                    className={articleStyles.emptyStateIcon}
                  />
              ...
              {templateStructure && (
                <div className={articleStyles.sectionContainer}>
                  <div className={articleStyles.sectionHeader}>
                    ...
                    >
                      <h4 className={articleStyles.sectionTitle}>
                        <TbListDetails
                          size={18}
                          className={articleStyles.sectionTitleIcon}
                        />
                        {t('articleGenerate_sectionConfiguration')}
                      </h4>
                  ...
                  {flattenSections(templateStructure).map(
                    ({ section, level, id, parentId }) => {
                      const isChild = level > 0;
                      return (
                        <div
                          key={id}
                          className={`${articleStyles.sectionItem} ${isChild ? articleStyles.sectionItemChild : ''}`}
                        >
                          {/* 섹션 헤더 */}
                          <div className={articleStyles.sectionItemHeader}>
                            ...
                            <div className={articleStyles.sectionActions}>
                              {!isChild && (
                                <Tooltip
                                  content={t('articleGenerate_addSubsection')}
                                >
                                  <div
                              ...
                              <Tooltip
                                content={t('articleGenerate_deleteSection')}
                              >
                                <div
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              handleIdeaChange(section.id!, e.target.value)
                            }
                            rows={3}
            ...
            <div className={articleStyles.referenceSection}>
              <h3 className={articleStyles.referenceSectionTitle}>
              ...
              <div className={styles.formGroup}>
                ...
                >
                  <div
                    className={tagSelectorStyles.tagFilterContainer}
                  ...
                  >
                    ...
                    >
                      {selectedWritingStyleId
                        ? writingStyles.find(
                            (ws) => ws.id === selectedWritingStyleId
                          )?.name || t('articleGenerate_writeStyleSelection')
                    ...
                    <div
                      className={`${tagSelectorStyles.tagFilterDropdown} ${isStyleDropdownOpen ? tagSelectorStyles.visible : ''}`}
                    >
                      <div
            ...
            <div className={articleStyles.referenceSection}>
              <h3 className={articleStyles.referenceSectionTitle}>
                {t('articleGenerate_referenceMaterials')}
              </h3>
              <button
                ...
                onClick={async () => {
                  ...
                  backupSelectedUploadsRef.current = JSON.parse(
                    JSON.stringify(selectedUploads)
                  );
              ...
              <div className={articleStyles.referenceList}>
                {selectedScraps.map((scrap) => (
                  <div
                    key={scrap.scrapId}
                    className={articleStyles.referenceItem}
                  >
                    <div style={{ flex: 1 }}>
                      <div>{scrap.title}</div>
                      <textarea
                        ...
                        onKeyDown={(e) => {
                          ...
                          const isDeletion = ['Backspace', 'Delete'].includes(
                            e.key
                          );
                          const willAddChar =
                            !isModifier &&
              ...
              {selectedUploads.length > 0 && (
                <div style={{ marginTop: 20 }}>
                  <h4 style={{ margin: 0, fontSize: 14, color: '#374151' }}>
                    {t('articleGenerate_selectedPDF')}
                  </h4>
                  <div
                    className={articleStyles.referenceList}
                    style={{ marginTop: 8 }}
                  >
                    {selectedUploads.map((u) => (
                      <div
                        key={u.uploadedFileId}
                        className={articleStyles.referenceItem}
                      >
                        <div style={{ flex: 1 }}>
                          <div>{u.title}</div>
                          <textarea
                            ...
                            placeholder={t(
                              'articleGenerate_howToUseThisMaterial'
                            )}
                          />
                          <div
                            style={{
                        ...
                        <button
                          ...
                          onClick={async () => {
                            setSelectedUploads((prev) =>
                              prev.filter(
                                (x) => x.uploadedFileId !== u.uploadedFileId
                              )
                            );
                            try {
                              await trackArticleReferenceRemovedBridge({
        ...
        >
          <div className={articleStyles.analysisModal}>
            <div className={articleStyles.modalHeader}>
              <h2 className={articleStyles.modalTitle}>
                {t('articleGenerate_aiPageAnalysis')}
              </h2>
              <button
            ...
            <div className={articleStyles.analysisModalContent}>
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                ...
                >
                  {t('articleGenerate_analysisDetailExplanation')
                    .split('\nBR')
                    .map((line, index) => (
                ...
                {templateStructure && templateStructure.length > 0 && (
                  ...
                  >
                    {t('articleGenerate_warningExistingSections')
                      .split('\nBR')
                      .map((line, index) => (
                        <React.Fragment key={index}>
                          {line}
      ...
      {isGenerating && (
        ...
        >
          <div className={articleStyles.analysisModal}>
            <div className={articleStyles.modalHeader}>
              <h2 className={articleStyles.modalTitle}>
                {t('articleGenerate_draftGenerationInProgress')}
              </h2>
            </div>
            <div className={articleStyles.analysisModalContent}>
              <div
              ...
              >
                ...
                >
                  {generationStatus === 'completed'
                    ...
                    : t('articleGenerate_processingRequest')}
                </h3>
                {generationStatus !== 'completed' && (
                  <p
                    style={{
                ...
                <div
                  style={{ marginTop: 8, color: '#6b7280', fontSize: '13px' }}
                >
                  {t('articleGenerate_elapsedTimeLabel')}:{' '}
      ...
      {isScrapModalOpen && (
        ...
        >
          ...
          >
            <div className={articleStyles.modalHeader}>
              <h2 className={articleStyles.modalTitle}>
                {t('articleGenerate_selectReferenceMaterials')}
              </h2>
            </div>
            {/* 탭 */}
            <div
              style={{ display: 'flex', gap: 8, padding: '0 16px 12px 16px' }}
            >
              <button
                className={articleStyles.sectionButton}
                style={{
                  background:
                    referenceModalTab === 'SCRAP' ? '#111827' : '#fff',
                  color: referenceModalTab === 'SCRAP' ? '#fff' : '#111827',
                }}
                onClick={() => setReferenceModalTab('SCRAP')}
              >
                {t('articleGenerate_scraps')}
              </button>
              <button
                className={articleStyles.sectionButton}
                style={{
                  background: referenceModalTab === 'PDF' ? '#111827' : '#fff',
                  color: referenceModalTab === 'PDF' ? '#fff' : '#111827',
                }}
                onClick={() => setReferenceModalTab('PDF')}
              >
                {t('articleGenerate_pdf')}
              </button>
            </div>
                  ...
                  {filteredScraps.map((scrap: ScrapResponse) => (
                    <div
                      ...
                      className={`${articleStyles.scrapItem} ${
                        selectedScraps.find((s) => s.scrapId === scrap.scrapId)
                          ? articleStyles.selected
                          : ''
                      }`}
                      onClick={() => handleScrapSelect(scrap)}
                      data-url={scrap.url}
                    >
                      <div className={articleStyles.scrapTitle}>
                        {scrap.title}
                      </div>
                      ...
                      <div className={articleStyles.scrapFooter}>
                        <div className={articleStyles.scrapTags}>
                          <TagList
                            tags={scrap.tags?.map((tag) => tag.name) || []}
                          />
                        </div>
                        <div className={articleStyles.scrapDate}>
                          {formatScrapDate(scrap.createdAt)}
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              {referenceModalTab === 'PDF' && (
                <>
                  {filteredUploads.length === 0 ? (
                    <div className={articleStyles.emptyState}>
                      <p className={articleStyles.emptyStateSubtitle}>
                        {t('articleGenerate_noPDFsFound')}
                      </p>
                    </div>
                  ) : (
                    filteredUploads.map((upload) => {
                      const selected = selectedUploads.find(
                        (u) => u.uploadedFileId === upload.id
                      );
                      return (
                        <div
                        ...
                        >
                          ...
                          <div className={articleStyles.scrapContent}>
                            {upload.previewText
                              ? upload.previewText.length > 100
                                ? `${upload.previewText.substring(0, 100)}...`
                                : upload.previewText
                              : t('articleGenerate_noDescription')}
                          </div>
                          <div className={articleStyles.scrapFooter}>
                            <div className={articleStyles.scrapTags}>
                            ...
                            <div className={articleStyles.scrapDate}>
                              {formatScrapDate(upload.createdAt)}
                            </div>
                          </div>
                        </div>
                      );
            ...
            <div className={articleStyles.modalFooter}>
              <button
                ...
                onClick={() => {
                  ...
                  setSelectedUploads(
                    JSON.parse(JSON.stringify(backupSelectedUploadsRef.current))
                  );
                  toggleScrapModal();
                }}
              >
